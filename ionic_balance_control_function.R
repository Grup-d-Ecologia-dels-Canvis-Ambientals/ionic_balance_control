...
# inaccurate measurements. The function assigns two quality flags according to a ionic balance threshold 
...
# IB_thres = Ionic balance threshold. Samples with a ionic balance less than or equal to this threshold (in absolute value) 
...
# units_ueq = TRUE (default) or FALSE  
      # Major anions (Cl, SO4, NO3), major cations (Na, K, Ca, Mg) ammonium and nitrites can be 
      # expressed either in µeq/L or in mg/L, but all must be in the same unit.
...
ionic_balance_control<-function(pH, ALK, Ca, Cl, K, Mg, Na, NH4, NO3, NO2, SO4, Cond, IB_thres, ALK.Dif_thres = NULL, units_ueq = TRUE){
  ...
  if (!units_ueq) {
  # Let's convert all the units to equivalents
  H = 10^6 * 10^-(pH)
  ...
  # Now we search for the summatory of cations and anions and the corresponding ionic balance
...
# way to join the result at the original table:
